...
public class InventoryTab extends Tab{
	...
	private Label selectedEquipTitle;
	private Label[] selectedEquipDesc = new Label[12];
	...
	public InventoryTab(){
		...
		selectedEquipTitle = new Label("", ReftGame.font);
		
		selectedEquipTitle.setPosition(ReftGame.getViewWidth()/2 + 10, 216);
		this.addActor(selectedEquipTitle);
		
		for (int i = 0; i < selectedEquipDesc.length; i++) {
			selectedEquipDesc[i] = new Label("", ReftGame.font);
			selectedEquipDesc[i].setPosition(ReftGame.getViewWidth()/2 + 10, 200 - 16 - i * 16);
			this.addActor(selectedEquipDesc[i]);
		}
	...
	public void update(Entity p){
		...
		if (selectedItem != null){
			...
			this.selectedEquipTitle.setText(selectedItem.getThing().name);
			for (int i = 0; i < this.selectedEquipDesc.length; i++) {
				this.selectedEquipDesc[i].setText(selectedItem.getThing().desc[i]);
			}
		} else {
			this.selectedEquipTitle.setText("");
			for (int i = 0; i < this.selectedEquipDesc.length; i++) {
				this.selectedEquipDesc[i].setText("");
			}
		}
